...
class BooleanExpression_AST:
    ...
    def true_code(self, label):
        comps = ""
        for comp in self.expressions:
            comps += comp.true_code(label)
        return comps
    def false_code(self, label):
        l1 = label_generator.next()
        comps = ""
        for comp in self.expressions:
            comps += comp.true_code(l1)
        return comps + 'goto ' + label + '\n' + l1 + ':\n'
...
class  BooleanFactor_AST:
    ...
    def __repr__(self):
        pass
    def true_code(self, label):
        return self.factors.false_code(label)
    def false_code(self, label):
        return self.factors.true_code(label)
...
class BooleanTerm_AST:
    ...
    def true_code(self, label):
        l1 = label_generator.next()
        comps = ''
        for comp in self.terms:
            comps += comp.false_code(l1)
        return comps + 'goto ' + label + '\n' + l1 + ':\n'
    def false_code(self, label):
        comps = ''
        for comp in self.terms:
            comps += comp.false_code(label)
        return comps
...
def boolean_expression():
    ...
    while scanner.lookahead() == Token.OR:
        ...
        exps.append(boolean_term())
    return BooleanExpression_AST(exps)

def boolean_term():
    terms = [boolean_factor()]
    while scanner.lookahead() == Token.AND:
        scanner.consume(Token.AND)
        terms.append(boolean_factor())
    return BooleanTerm_AST(terms)
